	...
	return function (_Component) {
		...
		_createClass(XDebugControl, [{
			...
			value: function render() {
				...
				if (!this.props.site.running || this.props.site.disconnected) {
					return React.createElement(
						OutputArea,
						null,
						React.createElement(Title, { text: 'Machine is not running' })
					);
				}
				...
				if (this.props.site.hasError) {
					return React.createElement(Error, { source: 'Site Container', message: this.props.site.error || 'n/a' });
				}
				...
				if (['active', 'inactive'].indexOf(xdebugStatus) === -1) {
					return React.createElement(
						...
						React.createElement(StatusString, { text: 'XDebug status is undefined' })
					);
				}
				...
				return React.createElement(
					...
					React.createElement(
						OutputSection,
						null,
						React.createElement(StatusString, { status: xdebugStatus, text: 'XDebug is ' + xdebugStatus }),
						button
					),
					React.createElement(
						OutputSection,
			...
			value: function componentDidMount() {
				if (!(this.props.site && this.props.site.running && this.props.container)) {
					return;
				}
