		...
		_createClass(XDebugControl, [{
			...
			value: function render() {
				...
				if (this.props.site.hasError) {
					return React.createElement(Error, { source: 'Site Container', message: site.error });
				}
				...
				if (container.environment !== 'custom') {
					...
					return React.createElement(
						...
						React.createElement(
							OutputSection,
							null,
							React.createElement(XDebugFieldList, { applyWith: container.applyXdebugSettings.bind(container) })
						)
					);
				}
			}
