	...
	return function () {
		...
		_createClass(Container, [{
			...
			value: function getXdebugStatus() {
				try {
					var status = this.exec('wget -qO- localhost/local-phpinfo.php | grep Xdebug');
					if (status.length !== 0) {
						return 'active';
					}
					return 'inactive';
					...
					var pid = err.pid;
					var signal = err.signal;
					var _status = err.status;
					return 'inactive';
				}
			}
		}, {
			...
			value: function activateXdebug() {
				...
				var activated = this.exec('sed -i \'/^;zend_extension.*xdebug.so/ s/;zend_ex/zend_ex/\' ' + phpIniFile);
			...
			value: function deactivateXdebug() {
				var phpIniFile = this.getSitePhpIniFilePath();
				var deactivated = this.exec('sed -i \'/^zend_extension.*xdebug.so/ s/zend_ex/;zend_ex/\' ' + phpIniFile);
				this.restartPhpService();
			}
		}, {
