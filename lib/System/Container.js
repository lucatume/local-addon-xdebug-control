...
module.exports = function () {
	...
	return function () {
		...
		_createClass(Container, [{
			...
			key: 'readXdebugStatus',
			value: function readXdebugStatus() {
				...
				var commands = settingsReadCommands.concat(this.xdebugStatusReadCommand()).concat(silenceCommand(this.remoteHostSetCommand()));
			...
			value: function activateXdebug() {
				...
				var commands = ['sed -i \'/^;zend_extension.*xdebug.so/ s/;zend_ex/zend_ex/\' ' + phpIniFile, silenceCommand(this.phpRestartCommand())].concat(this.xdebugStatusReadCommand());
			...
			value: function remoteHostSetCommand() {
				var remoteHostVarCommand = 'export REMOTE_HOST=$(ifconfig eth1 | grep \'inet addr:\' | cut -d: -f2 | awk \'{print $1}\')';
