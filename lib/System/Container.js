	...
	return function () {
		...
		_createClass(Container, [{
			...
			value: function xdebugSettingUpdateCommand(setting, value) {
				var iniFile = this.maps.phpIniFile(this.phpVersion);
				var updateIt = 'sed "/^xdebug.' + setting + '/ s/xdebug.' + setting + '.*/xdebug.' + setting + '=' + value + '/" ' + iniFile + ' >ini.tmp && mv ini.tmp ' + iniFile;
				var createIt = 'sed "/^zend_extension.*xdebug.so/ s/xdebug.so/xdebug.so\\nxdebug.' + setting + '=' + value + '/" ' + iniFile + ' >ini.tmp  && mv ini.tmp ' + iniFile;
			...
			value: function activateXdebug() {
				var iniFile = this.getSitePhpIniFilePath();
				var commands = ['sed \'/^;zend_extension.*xdebug.so/ s/;zend_ex/zend_ex/\' ' + iniFile + ' >ini.tmp && mv ini.tmp ' + iniFile, silenceCommand(this.phpRestartCommand())].concat(this.xdebugReadStatusAndSettingsCommands());
